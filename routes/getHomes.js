...
module.exports = {
	...
	handler: async (req, res) => {
		...
		req.query.garden ? findQuery.push({garden: req.query.garden}): findQuery = findQuery;
		req.query.swimming_pool ? findQuery.push({swimming_pool: req.query.swimming_pool}): findQuery = findQuery;
		req.query.air_conditioning ? findQuery.push({air_conditioning: req.query.air_conditioning}): findQuery = findQuery;
		req.query.heating ? findQuery.push({heating: req.query.heating}): findQuery = findQuery;
		req.query.orientation ? findQuery.push({orientation: req.query.orientation}): findQuery = findQuery;
		req.query.energy_certificate ? findQuery.push({energy_certificate: req.query.energy_certificate}): findQuery = findQuery;
		...
		if(findQuery.length > 0){
			...
			try {
				const homes = await homeSchema.find().skip(pageOptions.page * pageOptions.limit).limit(pageOptions.limit).sort(sorting)
				return res.response(homes);
			} catch (error) {
				return res.response(error).code(500);
			}
